...
export default function ScoreEntry() {
  ...
  async function loadSeason() {
    ...
    const w = await store.weeks.getBySeason(s.id)
    setAllWeeks(w)
    ...
    const enriched = allMatchups.map(m => {
      ...
      const entries = ['home', 'away'].flatMap(side => {
        ...
        return ['A', 'B'].map(slot => {
          ...
          return {
            matchupId: m.id, playerId: player.id, playerName: player.name,
            teamNumber: side === 'home' ? homeTeam?.number : awayTeam?.number,
            slot, side,
            handicap: hcpMap[player.id]?.value ?? null,
        ...
        >
          {allWeeks.filter(w => w.week_type !== 'holiday').map(w => (
            <option key={w.id} value={w.id}>
              Week {w.number} — {fmtDate(w.date)}{w.week_type !== 'regular' ? ` (${fmtType(w.week_type)})` : ''}
            </option>
          ))}
        </select>
      </div>
  ...
  return (
    <div className="bg-white rounded-xl shadow-sm overflow-hidden">
      <div className="bg-green-800 text-white px-4 py-2.5 flex items-center justify-between">
        ...
        <span className="text-sm">Team {matchup.homeTeam?.number} <span className="text-green-300">vs</span> Team {matchup.awayTeam?.number}</span>
          ...
          return (
            <div key={slot} className="px-4 py-3 flex items-start gap-2">
              ...
              <div className="flex-1 grid grid-cols-2 gap-3">
                <PlayerInput entry={h} result={result === 'home' ? 'win' : result === 'tie' ? 'tie' : result === 'away' ? 'loss' : null}
                  onChange={(f, v) => h && onUpdate(matchup.id, h.playerId, f, v)} />
                <PlayerInput entry={a} result={result === 'away' ? 'win' : result === 'tie' ? 'tie' : result === 'home' ? 'loss' : null}
...
function PlayerInput({ entry, result, onChange }) {
  if (!entry) return <div />
  const bgColor = result === 'win' ? 'bg-green-50 border-green-300' : result === 'loss' ? 'bg-red-50 border-red-200' : result === 'tie' ? 'bg-yellow-50 border-yellow-200' : 'border-gray-200'
  const net = (() => { const g = entry.isBlind ? null : parseInt(entry.gross); return isNaN(g) || g == null ? null : netScore(g, entry.handicap ?? 0) })()
  ...
  return (
    <div className={`rounded-lg border p-2 ${bgColor}`}>
      ...
      <div className="flex items-center gap-1.5">
        <input type="number" inputMode="numeric" value={entry.isBlind ? '' : entry.gross}
          onChange={e => onChange('gross', e.target.value)} disabled={entry.isBlind}
        ...
        <label className="flex items-center gap-1 cursor-pointer">
          <input type="checkbox" checked={entry.isBlind} onChange={e => onChange('isBlind', e.target.checked)} className="w-3.5 h-3.5 accent-green-700" />
...
function fmtType(t) { return { regular: 'Regular', position_night: 'Position Night', scramble: 'Scramble', end_scramble: 'End Scramble', holiday: 'Holiday' }[t] || t }
