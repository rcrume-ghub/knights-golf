...
// protect: null = public, 'admin' = admin+, 'commissioner' = commissioner only
const TABS = [
  { path: '', label: 'Overview', protect: null },
  { path: 'roster', label: 'Roster', protect: null },
  { path: 'schedule', label: 'Schedule', protect: null },
  { path: 'scores', label: 'Enter Scores', protect: 'admin' },
  { path: 'stats', label: 'Stats', protect: null },
  { path: 'dues', label: 'Dues', protect: null },
  { path: 'subs', label: 'Subs', protect: null },
  { path: 'email', label: 'Email', protect: 'commissioner' },
  { path: 'league-rules', label: 'League Rules', protect: null },
  { path: 'settings', label: 'Settings', protect: 'commissioner' },
]

export default function Season() {
  const { seasonId } = useParams()
  const navigate = useNavigate()
  const { isAdmin, isCommissioner } = useAuth()
  ...
  function hasAccess(protect) {
    if (!protect) return true
    if (protect === 'admin') return isAdmin
    if (protect === 'commissioner') return isCommissioner
    return false
  }

  function handleLockedTab(path) {
    setLoginTarget(path)
    setShowLogin(true)
  }
  ...
  function onLoginSuccess() {
    if (loginTarget != null) {
      const dest = loginTarget === '' ? `/season/${seasonId}` : `/season/${seasonId}/${loginTarget}`
      navigate(dest)
    }
    setLoginTarget(null)
  }
      ...
      <div className="flex flex-col min-h-full">
        ...
        <div className="bg-white border-b border-gray-200 px-4 pt-3 pb-0">
          ...
          <div className="flex gap-1 overflow-x-auto pb-0 -mx-4 px-4 no-scrollbar">
            {TABS.map(t => {
              const accessible = hasAccess(t.protect)
              const dest = t.path === '' ? `/season/${seasonId}` : `/season/${seasonId}/${t.path}`

              if (!accessible) {
                return (
                  <button
                    key={t.path}
                    onClick={() => handleLockedTab(t.path)}
                    className="flex-shrink-0 px-3 py-2.5 text-xs font-semibold border-b-2 border-transparent text-gray-300 whitespace-nowrap"
                  >
                    🔒 {t.label}
                  </button>
                )
